try {
    ...
    inputs.forEach(function (input) {
        ...
        for (let index = 0; index < input.length; index++) {
            ...
            assert.sameValue(true, result.index < input.length, msg + 'index < ' + input.length);
            assert.sameValue('string', typeof result.input, msg + 'input');
            assert.sameValue('boolean', typeof result.isWordLike, msg + 'isWordLike should be boolean');
        }
    });
} catch (e) {
    try {
