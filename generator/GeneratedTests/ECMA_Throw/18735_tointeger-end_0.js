try {
    ...
    testWithTypedArrayConstructors(function (TA) {
        ...
        assert(compareArray(sample.slice(0, undefined), [
            40,
            41,
            42,
            43
        ...
        assert(compareArray(sample.slice(0, -0.6), []), '-0.6');
        assert(compareArray(sample.slice(0, -1.1), [
            40,
            41,
            42
        ]), '-1.1');
        assert(compareArray(sample.slice(0, -1.5), [
            40,
            41,
            42
        ]), '-1.5');
        assert(compareArray(sample.slice(0, '3'), [
            40,
            41,
            42
        ]), 'string');
        assert(compareArray(sample.slice(0, obj), [
            40,
            41
        ]), 'object');
    });
